...
class DataGenerator():
    ...
    # the letters to randomly include in a sequence. We'll just assume ACTG
    # for simplicity
    LETTERS = ['A', 'T', 'C', 'G']
    ...
    def generate_DNA_sequence(self):
        sequence = "".join(random.choice(self.LETTERS)
                           for i in xrange(self.n - self.l))
        starting_point = random.randint(0, self.n - self.l)
        # insert the motif at a random spot
        sequence = sequence[:starting_point] + self.mutate(
            self.motif) + sequence[starting_point:]
        return sequence

    def mutate(self, motif):
        # keep track of where we mutated, since we can't mutate the same spot
        # twice. That only counts as one mutation! Or even 0 if we mutate back
        # to the orignal nucelotide!
        mutation_indicies = []
        mutated_motif = motif
        for i in xrange(self.mutations):
            ...
            # if we wanted to be REALLY efficient we could cache this list for
            # each letter but come on this is Python
            mutated_letter = random.choice(filter(
                lambda letter: letter != current_letter, self.LETTERS))
            # remember that we mutated this spot
            mutation_indicies.append(index)
            # finally mutate the letter
            mutated_motif = mutated_motif[
                :index] + mutated_letter + mutated_motif[index:]
